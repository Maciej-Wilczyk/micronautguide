...
public class NewsService {
    ...
    public List<String> addHeadline(Month month, List<String> headlineList) {
        if (headlines.containsKey(month)) {
            List<String> l = new ArrayList<>(headlines.get(month));
            l.addAll(headlineList);
            headlines.put(month, l);
        } else {
            headlines.put(month, headlineList);
        }
        return headlines.get(month);
    }

    @CacheInvalidate(parameters = {"month"}) // <6>
    public void removeHeadline(Month month, List<String> headline) {
        if (headlines.containsKey(month)) {
            List<String> l = new ArrayList<>(headlines.get(month));
            l.remove(headline);
            headlines.put(month, l);
